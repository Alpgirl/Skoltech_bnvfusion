...
@register("fusion_inference_dataset_sk3d")
class SkoltechInferenceDataset(FusionInferenceAbstractDataset):
    def __init__(self, cfg, stage):
        super().__init__(cfg, stage)
        data_dir = cfg.dataset.data_dir
        cam_dir = cfg.dataset.cam_params
        depth_dir = cfg.dataset.depth_dir
        self.gt_mesh = cfg.dataset.gt_mesh
        ...
        for cam_txt in sorted(os.listdir(cam_dir)):
            if cam_txt != "pair.txt":
                cam_path = os.path.join(cam_dir, cam_txt)
                self.intr_mat_paths.append(cam_path)
                self.T_wc_paths.append(cam_path)

        # read depth and mask
        for depth_name in sorted(os.listdir(depth_dir)):
    ...
    def read_depth(self, path):
        depth, _, mask = load_depth_sk3d(path, downsample_scale=self.downsample_scale,
                                    max_depth=self.max_depth, add_noise=False)
        return depth, mask
    ...
    def read_intr_pose(self, path):
        ...
        intr_mat = np.fromstring(' '.join(lines[7:10]), dtype=np.float32, sep=' ').reshape((3, 3))
        return intr_mat
